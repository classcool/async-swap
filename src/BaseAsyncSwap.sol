...
abstract contract BaseAsyncSwap is BaseHook, IHookEvents {
  ...
  {
    // Async swaps are only possible on exact-input swaps, so exact-output swaps are executed by the `PoolManager` as
    // normal
    if (params.amountSpecified < 0) {
      ...
      if (specified == key.currency0) {
        emit HookSwap(PoolId.unwrap(key.toId()), sender, specifiedAmount.toInt128(), 0, feeAmount.toUint128(), 0);
      } else {
        emit HookSwap(PoolId.unwrap(key.toId()), sender, 0, specifiedAmount.toInt128(), 0, feeAmount.toUint128());
      }

...
contract Router is IRouter {
    ...
    bytes32 constant ACTION_LOCATION =
        0xf3b150ebf41dad0872df6788629edb438733cb4a5c9ea779b1b1f3614faffc69;
    /// keccak256("Router.User") - 1
    bytes32 constant USER_LOCATION =
        0x3dde20d9bf5cc25a9f487c6d6b54d3c19e3fa4738b91a7a509d4fc4180a72356;
    /// keccak256("Router.AsyncFiller") - 1
    bytes32 constant ASYNC_FILLER_LOCATION =
        0xd972a937b59dc5cb8c692dd9f211e85afa8def4caee6e05b31db0f53e16d02e0;
    ...
    function swap(
        ...
        IAsyncCSMM.UserParams memory userParams = abi.decode(
            userData,
            (IAsyncCSMM.UserParams)
        );
        require(
            userParams.executor == address(this),
            "Use router as your executor!"
        );
    ...
    function fillOrder(
        ...
        poolManager.unlock(
            abi.encode(SwapCallback(ActionType.FillOrder, order))
        );
    }
    ...
    function unlockCallback(
        ...
        if (action == 0) {
            ...
            poolManager.swap(
                ...
                IPoolManager.SwapParams(
                    orderData.order.zeroForOne,
                    -orderData.order.amountIn.toInt256(),
                    orderData.order.sqrtPrice
                ),
                abi.encode(user, asyncFiller)
            );
            Currency specified = orderData.order.zeroForOne
                ? orderData.order.key.currency0
                : orderData.order.key.currency1;
            specified.settle(
                poolManager,
                user,
        ...
        if (action == 1) {
            SwapCallback memory orderData = abi.decode(data, (SwapCallback));
            Currency currency = orderData.order.zeroForOne
                ? orderData.order.key.currency1
                : orderData.order.key.currency0;
            assert(
            ...
            assert(
                IERC20Minimal(Currency.unwrap(currency)).approve(
                    address(hook),
                    orderData.order.amountIn
                )
            );

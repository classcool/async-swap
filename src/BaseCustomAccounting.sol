...
abstract contract BaseCustomAccounting is BaseHook, IHookEvents, IUnlockCallback {
    ...
    returns (BalanceDelta delta)
  {
    ...
    if (isNative) {
      // Check that delta amount was covered by msg.value given that settle would be valid if hook can pay for
      // difference
   ...
   * @return callerDelta The balance delta from the liquidity modification. This is the total of both principal and fee
   * deltas.
    ...
    returns (BalanceDelta callerDelta, BalanceDelta feesAccrued)
  {
    (callerDelta, feesAccrued) = abi.decode(
      poolManager.unlock(abi.encode(CallbackData(msg.sender, abi.decode(params, (IPoolManager.ModifyLiquidityParams))))),
    ...
    returns (bytes memory returnData)
  {
    ...
    // Set the salt value of the liquidity position, which is the keccak256 hash of the sender and salt from the
    // callback data
   ...
   *
   * @param data The encoded `CallbackData` struct, including the sender and the parameters for the liquidity
   * modification.

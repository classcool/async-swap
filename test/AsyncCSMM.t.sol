...
contract AsyncCsmmTest is SetupHook {
    ...
    function testAsyncSwap() public {
        ...
        if (zeroForOne) {
            assertEq(
                manager.balanceOf(user, currency0.toId()),
                uint256(amount)
            );
        } else {
            assertEq(
                manager.balanceOf(user, currency1.toId()),
                uint256(amount)
            );
        }
    }

    function testFuzzAsyncSwapOrder(
        bool zeroForOne,
        uint256 amount,
        ...
        if (zeroForOne) {
            assertEq(
                currency0.balanceOf(user),
                userCurrency0Balance - uint256(amount)
            );
            assertEq(currency1.balanceOf(user), userCurrency1Balance);
        } else {
            assertEq(
                currency1.balanceOf(user),
                userCurrency1Balance - uint256(amount)
            );
        ...
        if (zeroForOne) {
            assertEq(
                ...
                balance0Before + uint256(amount)
            );
        } else {
            assertEq(
                manager.balanceOf(address(hook), currency1.toId()),
                balance1Before + uint256(amount)
            );
        }
